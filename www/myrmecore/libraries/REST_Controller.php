...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
		...
		{
			$this->response( array('result' => false, 'error' => 'Only AJAX requests are accepted.'), 505 );
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
      ...
      if (config_item('rest_enable_logging') AND $log_method)
			...
			$this->response(array('result' => false, 'error' => 'INVALID_API_KEY'), 403);
		}

		// Sure it exists, but can they do anything with it?
		if ( ! method_exists($this, $controller_method))
		{
			$this->response(array('result' => false, 'error' => 'UNKNOWN_METHOD'), 404);
		}
		...
		if (config_item('rest_enable_keys') AND ! empty($this->rest->key))
		{
			...
			{
				$this->response(array('result' => false, 'error' => 'API_CALLS_HOURLY_LIMIT_REACHED'), 401);
			}
			...
			$authorized OR $this->response(array('result' => false, 'error' => 'NOT_ENOUGH_PERMISSIONS'), 401);
		}
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('result' => false, 'error' => 'UNAUTHORIZED'), 401);
		}
	}
